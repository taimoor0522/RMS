...
// Get staff_id - if admin, use first available staff
if (isset($_SESSION['staff_id']) && !empty($_SESSION['staff_id'])) {
    $staff_id = $_SESSION['staff_id'];
} else {
    // For admin users, get the first staff_id from database
    $ret = "SELECT staff_id FROM rpos_staff ORDER BY staff_id ASC LIMIT 1";
    $stmt = $mysqli->prepare($ret);
    if ($stmt) {
        $stmt->execute();
        $res = $stmt->get_result();
        if ($res->num_rows > 0) {
            $staff_row = $res->fetch_object();
            $staff_id = $staff_row->staff_id;
        } else {
            // If no staff exists, show error
            $staff_id = null;
        }
    } else {
        $staff_id = null;
...
if (isset($_POST['createOrder'])) {
  if (empty($_POST["supplier_id"]) || empty($_POST['item_id']) || empty($_POST['item_qty'])) {
    ...
    $err = "Cannot create order: No staff member available. Please add a staff member first.";
  } else {
...
<body>
            ...
            <div class="table-responsive">
              <table class="table align-items-center table-flush">
                ...
                <tbody>
                  ...
                  $table_check = $mysqli->query("SHOW TABLES LIKE 'rpos_supplier_orders'");
                  if ($table_check->num_rows == 0) {
                    echo '<tr><td colspan="9" class="text-center">
                      <div class="alert alert-warning">
                        <h4>Supplier orders table not found!</h4>
                  ...
                  <?php 
                        ...
                        echo '<tr><td colspan="9" class="text-center text-muted">No orders found. Create your first order above.</td></tr>';
                      }
                    } else {
                      echo '<tr><td colspan="9" class="text-center text-danger">Error loading orders: ' . $mysqli->error . '</td></tr>';
                    }
                  }
                  ?>
                </tbody>
  ...
  <script>
    ...
    // Load items on page load if supplier is preselected
